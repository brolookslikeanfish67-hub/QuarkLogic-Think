...
def wait_for_async_backward_comm(backward_handle_dicts):
    def hook(grad):
        # Wait for backward communication handles
        for handle_dict in backward_handle_dicts:
            if 'backward' in handle_dict and handle_dict['backward'] is not None:
                ...
                handle_dict['backward'] = None
        return grad
    return hook


def set_tensor_sequence_number(tensor, value):
...
class SimpleFarSkipTransformerLayer(TransformerLayer):
    ...
    def __init__(
        ...
        self.attn_only_farskip = config.attn_only_farskip
        self.mlp_only_farskip = config.mlp_only_farskip
...
class OverlappedFarSkipTransformerLayer(TransformerLayer):
    ...
    def forward(self, *args, **kwargs):
        ...
        if combine_handle is None:
            ...
            if self.config.farskip_overlap_associative_add:
        ...
        set_tensor_sequence_number(input_to_dispatch, 0)
        set_tensor_sequence_number(new_probs_pre, 0)
        set_tensor_sequence_number(input_to_shared, 0)
        [set_tensor_sequence_number(some_tensor, 2) for some_tensor in forward_a_outputs if torch.is_tensor(some_tensor)]
        set_tensor_sequence_number(residual, 1)
        set_tensor_sequence_number(dispatched_input, torch.iinfo(torch.int).max)
        set_tensor_sequence_number(new_probs, torch.iinfo(torch.int).max)
        ...
        set_tensor_sequence_number(output, 3)
        set_tensor_sequence_number(new_combined_experts, torch.iinfo(torch.int).max)

        set_tensor_sequence_number(dispatched_input_post, torch.iinfo(torch.int).max)
        set_tensor_sequence_number(tokens_per_expert, torch.iinfo(torch.int).max)
        set_tensor_sequence_number(permuted_probs, torch.iinfo(torch.int).max)

        set_tensor_sequence_number(expert_output, torch.iinfo(torch.int).max)
        if mlp_bias is not None:
            set_tensor_sequence_number(mlp_bias, torch.iinfo(torch.int).max)
