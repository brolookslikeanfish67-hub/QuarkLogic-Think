...
# -------------------- Argument Parsing --------------------
# Defaults to the pretrain task (primus CLI) when neither --task nor TASK is set.
TASK="${TASK:-pretrain}"
...
# ----------- DPO Ref Logprobs Mode -----------
# When --compute-ref-logprobs is passed (dpo only), create a temp copy of the YAML
# with compute_ref_logprobs flipped to true, then launch against it. The temp file
# is cleaned up on exit. Run this step before DPO training so the reference
# log-probs IndexedDataset exists. prepare_experiment already ran on the original
# EXP above; the only difference is the compute_ref_logprobs flag.
...
# Select the entrypoint by task. pretrain goes through the primus CLI (--config);
# docmask/sft/dpo use the standalone per-script entrypoints (--exp) so each can
# detect its top-level config block. Unknown $TRAIN_EXTRA_ARGS are absorbed by
# parse_args as overrides in every case.
case "$TASK" in
    pretrain)
