...
def grouped_topk_gpu_instella(
    ...
    num_token = scores.shape[0]
    num_experts = scores.shape[1]
    scores_for_choice = scores + selection_bias.unsqueeze(0)
    group_scores = (
        scores_for_choice.view(num_token, num_expert_group, -1).max(dim=-1).values
    )  # [n, n_group]
    group_idx = torch.topk(group_scores, k=topk_group, dim=-1, sorted=False)[
        1
    ]  # [n, top_k_group]
    ...
    topk_weights = scores.gather(1, topk_ids)  # raw sigmoid, bias stripped
...
def select_experts(
    ...
    *,
    ...
    if use_grouped_topk:
        ...
        elif num_expert_group == 1:
            topk_weights, topk_ids = grouped_topk_gpu_instella(
                hidden_states=hidden_states,
                gating_output=router_logits,
                topk=num_routed_topk if _use_aiter else top_k,
                renormalize=renormalize,
                selection_bias=correction_bias,
                num_expert_group=num_expert_group,
                topk_group=topk_group,
                num_fused_shared_experts=num_fused_shared_experts,
                routed_scaling_factor=routed_scaling_factor,
